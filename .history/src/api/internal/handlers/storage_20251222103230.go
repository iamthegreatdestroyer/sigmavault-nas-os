...
func (h *StorageHandler) ListPools(c *fiber.Ctx) error {
	...
	if h.rpcClient != nil && h.rpcClient.IsConnected() {
		rpcPools, err := h.rpcClient.ListPools(c.Context())
		if err == nil {
			...
			for _, rp := range rpcPools {
				pools = append(pools, models.StoragePool{
					...
					FreeBytes:  rp.FreeBytes,
					UsedPct:    rp.UsedPercent,
				})
			}
...
func (h *StorageHandler) CreatePool(c *fiber.Ctx) error {
	...
	// Try RPC engine first
	if h.rpcClient != nil && h.rpcClient.IsConnected() {
		rp, err := h.rpcClient.CreatePool(c.Context(), req.Name, req.Disks, raidType)
		if err == nil {
...
func (h *StorageHandler) DeletePool(c *fiber.Ctx) error {
	...
	// Try RPC engine first
	if h.rpcClient != nil && h.rpcClient.IsConnected() {
		err := h.rpcClient.DeletePool(c.Context(), poolID)
		if err == nil {
			return c.JSON(fiber.Map{
...
func (h *StorageHandler) ListShares(c *fiber.Ctx) error {
	// Try RPC engine first
	if h.rpcClient != nil && h.rpcClient.IsConnected() {
		rpcShares, err := h.rpcClient.ListShares(c.Context())
		if err == nil {
...
func (h *StorageHandler) CreateShare(c *fiber.Ctx) error {
	...
	// Try RPC engine first
	if h.rpcClient != nil && h.rpcClient.IsConnected() {
		rs, err := h.rpcClient.CreateShare(c.Context(), req.Name, req.Path, req.Type, req.ReadOnly)
		if err == nil {
			return c.Status(fiber.StatusCreated).JSON(fiber.Map{

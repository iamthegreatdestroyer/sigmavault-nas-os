...
func (es *EventSubscriber) Start(ctx context.Context, pollInterval time.Duration) {
	...
	go es.run(ctx)
	log.Info().Duration("poll_interval", pollInterval).Msg("WebSocket event subscriber started")
}
...
func (es *EventSubscriber) pollCompressionJobs(ctx context.Context) {
	...
	for _, job := range jobs {
		data := map[string]interface{}{
			...
			"elapsed_seconds":  int64(job.ElapsedTime.Seconds()),
			"estimated_total":  int64(job.EstimatedTime.Seconds()),
			"timestamp":        time.Now().Unix(),
		}

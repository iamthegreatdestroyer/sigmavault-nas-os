...
import { useSystemMetrics, useAgents, useStoragePools } from '@hooks/useQueries';
...
type StatusBadgeStatus = "online" | "offline" | "idle" | "busy" | "error" | "warning";
...
function SystemMetrics() {
  ...
  return (
    <PageSection title="System Metrics" description="Real-time system resource utilization">
  ...
  return (
    ...
    >
      ...
      <Grid columns={4} gap="md">
        ...
        <StatCard
          title="Swarm Load"
          value={((agentStats.active / (agentStats.total || 1)) * 100).toFixed(0)}
          unit="%"
          icon={Zap}
          iconColor="bg-primary/20 text-primary"
          loading={isLoading}
...
function StorageOverview() {
  ...
  const avgCompression = pools?.length
    ? compressionRatio / pools.length
    : 0;
                ...
                <div key={pool.id} className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <StatusBadge status={mapStorageHealthToStatus(pool.status)} />
                    </div>
                    <span className="text-sm text-text-muted">
                      {formatBytes(pool.usedSpace)} / {formatBytes(pool.totalSpace)}
                    </span>
                  </div>
                  <Progress
...
function SecurityStatus() {
  return (
    <PageSection title="Security Status" description="Encryption and threat monitoring">

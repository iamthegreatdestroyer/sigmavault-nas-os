...
export const queryKeys = {
  ...
  network: {
    ...
    interface: (id: string) => [...queryKeys.network.all, 'interface', id] as const,
...
export const api = {
  ...
  agents: {
    ...
    deactivate: (id: string) =>
      request<ApiResponse<Agent>>(`/agents/${id}/deactivate`, { method: 'POST' }),
    
    assignTask: (id: string, task: { type: string; priority: number; payload: unknown }) =>
      request<ApiResponse<{ taskId: string }>>(`/agents/${id}/tasks`, {
        method: 'POST',
        body: JSON.stringify(task),
      }),
    
    getSwarmStatus: () => request<ApiResponse<{
      totalAgents: number;
      activeAgents: number;
      taskQueue: number;
      memoryUsage: number;
  ...
  storage: {
    ...
    getPool: (id: string) => request<ApiResponse<StoragePool>>(`/storage/pools/${id}`),
    ...
    // Volumes
    listVolumes: () => request<ApiResponse<StorageVolume[]>>('/storage/volumes'),
    
    getVolume: (id: string) => request<ApiResponse<StorageVolume>>(`/storage/volumes/${id}`),
    ...
    deleteVolume: (id: string) =>
      request<ApiResponse<void>>(`/storage/volumes/${id}`, { method: 'DELETE' }),

    // Disks
    listDisks: () => request<ApiResponse<StorageDisk[]>>('/storage/disks'),
    
    getDisk: (id: string) => request<ApiResponse<StorageDisk>>(`/storage/disks/${id}`),
    
    getSmart: (id: string) => request<ApiResponse<StorageDisk['smart']>>(`/storage/disks/${id}/smart`),
  },
  ...
  // --------------------------------------------------------------------------
  compression: {
    getStats: () => request<ApiResponse<CompressionStats>>('/compression/stats'),
    
    listJobs: () => request<ApiResponse<CompressionJob[]>>('/compression/jobs'),
    
    getJob: (id: string) => request<ApiResponse<CompressionJob>>(`/compression/jobs/${id}`),
    ...
    cancelJob: (id: string) =>
      request<ApiResponse<void>>(`/compression/jobs/${id}`, { method: 'DELETE' }),
  },
  ...
  // --------------------------------------------------------------------------
  network: {
    listInterfaces: () => request<ApiResponse<NetworkInterface[]>>('/network/interfaces'),
    ...
    // PhantomMesh VPN
    getPhantomMesh: () => request<ApiResponse<PhantomMeshNetwork>>('/network/phantommesh'),
    
    listPeers: () => request<ApiResponse<PhantomMeshPeer[]>>('/network/phantommesh/peers'),
    ...
    removePeer: (id: string) =>
      request<ApiResponse<void>>(`/network/phantommesh/peers/${id}`, { method: 'DELETE' }),
    
    enableMesh: () =>
      request<ApiResponse<PhantomMeshNetwork>>('/network/phantommesh/enable', { method: 'POST' }),
    
    disableMesh: () =>
      request<ApiResponse<void>>('/network/phantommesh/disable', { method: 'POST' }),
  },
  ...
  // --------------------------------------------------------------------------
  security: {
    getEncryptionStatus: () => request<ApiResponse<EncryptionStatus>>('/security/encryption'),
    
    rotateKeys: () =>
      request<ApiResponse<{ rotatedAt: string }>>('/security/encryption/rotate', { method: 'POST' }),
    
    getAuditLog: (params?: { limit?: number; offset?: number; level?: string }) => {
  ...
  system: {
    ...
    reboot: () => request<ApiResponse<void>>('/system/reboot', { method: 'POST' }),
    
    shutdown: () => request<ApiResponse<void>>('/system/shutdown', { method: 'POST' }),
  },
  ...
  auth: {
    login: (credentials: { username: string; password: string }) =>
      request<ApiResponse<{ token: string; expiresAt: string }>>('/auth/login', {
        method: 'POST',
        body: JSON.stringify(credentials),
      }),
    
    logout: () => request<ApiResponse<void>>('/auth/logout', { method: 'POST' }),
    
    refresh: () => request<ApiResponse<{ token: string; expiresAt: string }>>('/auth/refresh', {
      method: 'POST',
    }),
    
    me: () => request<ApiResponse<{
      id: string;
      username: string;
      role: string;
      permissions: string[];

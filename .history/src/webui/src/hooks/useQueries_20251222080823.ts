...
export function useAddPhantomMeshPeer() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.network.phantomMesh() });
    },
  });
}
...
export function useSecurityAudit(params?: Parameters<typeof api.security.getAuditLog>[0]) {
  return useQuery({
...
export function useRotateEncryptionKeys() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.security.encryption() });
    },
  });
}

...
class DashboardView(Gtk.Box):
    ...
    def start_auto_refresh(self) -> None:
        ...
        self._refresh_source_id = GLib.timeout_add(
            REFRESH_INTERVAL_MS, self._on_refresh_tick
        )
    ...
    def _fetch_system_status(self) -> None:
        ...
        async def _do_fetch():
            async with SigmaVaultAPIClient(
    ...
    def _on_status_received(self, status: Optional[SystemStatus]) -> None:
        ...
        cpu_class = "error" if status.cpu_percent > 90 else (
            "warning" if status.cpu_percent > 70 else "success"
        )
        ...
        mem_class = "error" if status.memory_percent > 90 else (
            "warning" if status.memory_percent > 70 else "success"
        )
        ...
        disk_class = "error" if status.disk_percent > 90 else (
            "warning" if status.disk_percent > 80 else "success"
        )

...
class AgentsView(Gtk.Box):
    ...
    def __init__(self, api_client: SigmaVaultAPIClient):
        ...
        self._total_agents_card = StatCard("system-run-symbolic", "Total Agents", "—", "Active in swarm")
        self._active_agents_card = StatCard("starred-symbolic", "Active", "—", "Currently processing")
        self._success_rate_card = StatCard("emblem-ok-symbolic", "Success Rate", "—", "Task completion")
    ...
    def _update_ui(self, agents) -> None:
        ...
        avg_response = sum(a.metrics.avg_response_time_ms for a in agents if a.metrics) / total if total > 0 else 0

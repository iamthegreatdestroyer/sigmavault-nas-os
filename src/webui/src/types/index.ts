...
export interface StorageDisk {
  ...
  smart: SmartStatus; // Alias for compatibility
  path: string;
}
...
export interface StoragePool {
  ...
  disks: StorageDisk[];
  health: StorageHealth;
  status: StorageHealth; // Alias for compatibility
  capacityBytes: number;
  usedBytes: number;
  usedSpace: number; // Alias for usedBytes
  totalSpace: number; // Alias for capacityBytes
...
export interface StorageVolume {
  ...
  poolId: string;
  mountPoint: string;
  filesystem: string; // e.g., "zfs", "ext4", "xfs"
  capacityBytes: number;
  usedBytes: number;
  quotaBytes?: number;
...
export interface CpuMetrics {
  usagePercent: number;
  usage: number; // Alias for usagePercent
  coreCount: number;
  frequency: number;
  temperature: number;
...
export interface MemoryMetrics {
  totalBytes: number;
  usedBytes: number;
  used: number; // Alias for usedBytes
  availableBytes: number;
  cachedBytes: number;
